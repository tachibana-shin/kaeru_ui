...
void main() {
  group('flex widget extension', () {
    test('alignment shorthands return Align with expected alignment', () {
      ...
      expect(
        (const Text('A').bottomLeft as Align).alignment,
        Alignment.bottomLeft,
      );
      expect(
        (const Text('A').bottomRight as Align).alignment,
    ...
    test('scrollable returns SingleChildScrollView with axis', () {
      final v =
          const Text('A').scrollable(axis: Axis.vertical)
              as SingleChildScrollView;
      expect(v.scrollDirection, Axis.vertical);
      final h =
          const Text('A').scrollable(axis: Axis.horizontal)
              as SingleChildScrollView;
      expect(h.scrollDirection, Axis.horizontal);
    });

    test(
      'expand and flexible return corresponding widgets with properties',
      () {
        final e = const Text('A').expand(flex: 2) as Expanded;
        expect(e.flex, 2);
        final f =
            const Text('A').flexible(flex: 3, fit: FlexFit.tight) as Flexible;
        expect(f.flex, 3);
